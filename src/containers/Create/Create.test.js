...
describe('Create tests', () => {
  ...
  it('should set state when clinic name input is changed', () => {
    const renderedApp = shallow(<Create user={mockUserData} />);
    const mockEvent = { target: { name: 'clinicName', value: 'I Need Therapy' }};
  ...
  it('should display a success message when saveNewClinic is called with correct params', () => {
    const renderedApp = shallow(<Create user={mockUserData} saveClinic={jest.fn()} />);
    const mockEvent = { preventDefault: jest.fn() };
    const mockClinicName = 'I Need Therapy';
    const mockClinicAbbreviation = 'INT';

    renderedApp.instance().setState({ clinicName: 'I Need Therapy', clinicAbbreviation: 'INT' });
    renderedApp.instance().saveNewClinic(mockEvent, mockClinicName, mockClinicAbbreviation);
