import React from 'react';
import './SessionsComparison.css';
import { connect } from 'react-redux';
import ComparisonCard from '../ComparisonCard/ComparisonCard';

const SessionsComparison = (props) => {

  let modulationDisplay;
  let posturalDisplay;
  let sensoryDisplay;
  let socialDisplay;
  let executiveDisplay;

  if (Object.keys(props.processesData).length) {
    const dateArray = props.processesData.modulation[0].values;

    modulationDisplay = props.processesData.modulation.map((obj, index) => {
      if (index > 0) {
        return (
          <ComparisonCard 
            dateArray={dateArray}
            title={obj.title}
            values={obj.values}
          />
        )
      }
    })

    posturalDisplay = props.processesData.postural.map((obj, index) => {
      if (index > 0) {
        return (
          <ComparisonCard 
            dateArray={dateArray}
            title={obj.title}
            values={obj.values}
          />
        )
      }
    })

    sensoryDisplay = props.processesData.sensory.map((obj, index) => {
      if (index > 0) {
        return (
          <ComparisonCard 
            dateArray={dateArray}
            title={obj.title}
            values={obj.values}
          />
        )
      }
    })

    socialDisplay = props.processesData.social.map((obj, index) => {
      if (index > 0) {
        return (
          <ComparisonCard 
            dateArray={dateArray}
            title={obj.title}
            values={obj.values}
          />
        )
      }
    })

    executiveDisplay = props.processesData.executive.map((obj, index) => {
      if (index > 0) {
        return (
          <ComparisonCard 
            dateArray={dateArray}
            title={obj.title}
            values={obj.values}
          />
        )
      }
    })
  }


  return (
    <div className="SessionsComparison">
      <h3>Sessions Comparison</h3>

      <h4>Modulation</h4>
      <div className="comparison-section-holder">
        {modulationDisplay}
      </div>

      <h4>Postural</h4>
      <div className="comparison-section-holder">
        {posturalDisplay}
      </div>

      <h4>Sensory</h4>
      <div className="comparison-section-holder">
        {sensoryDisplay}
      </div>

      <h4>Social</h4>
      <div className="comparison-section-holder">
        {socialDisplay}
      </div>

      <h4>Executive</h4>
      <div className="comparison-section-holder">
        {executiveDisplay}
      </div>

    </div>
  )
}

const mapStateToProps = store => ({
  processesData: store.processesData
})

export default connect(mapStateToProps, null)(SessionsComparison);